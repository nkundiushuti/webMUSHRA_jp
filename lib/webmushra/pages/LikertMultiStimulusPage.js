...
LikertMultiStimulusPage.prototype.init = function (_callbackError) {
  ...
  var cbk = (function(_prefix) {
    this.completedQuestionnaires[_prefix] = true;
    if (Object.keys(this.completedQuestionnaires).length == this.stimuli.length) {
  ...
  for (var i = 0; i < this.stimuli.length; ++i) {    
    this.stimulusQuestionnaires[i] = new LikertScale(this.pageConfig.response, i + "_", this.pageConfig.mustPlayback, cbk);
  }
  ...
  if (this.pageConfig.mustPlayback) {
    this.filePlayer.genericAudioControl.addEventListener((function (_event) {
      if (_event.name == this.pageConfig.mustPlayback) {
        this.stimulusQuestionnaires[_event.index].enable();
      } 
    }).bind(this));
...
LikertMultiStimulusPage.prototype.render = function (_parent) {  
  ...
  for (var i = 0; i < this.stimuli.length; ++i) {
    this.stimulusQuestionnaires[i].render(this.filePlayer.getHook(i));     
  }
...
LikertMultiStimulusPage.prototype.load = function () {
  ...
  for(var i = 0; i < this.stimuli.length; ++i){
  	if(this.responses[i]){
      $("input[name='"+this.stimulusQuestionnaires[i].prefix +"_response'][value='"+this.responses[i]+"']").attr("checked", "checked");
      $("input[name='"+this.stimulusQuestionnaires[i].prefix +"_response'][value='"+this.responses[i]+"']").checkboxradio("refresh");
      this.stimulusQuestionnaires[i].group.change();
    }
  }
...
LikertMultiStimulusPage.prototype.save = function () {
  ...
  for(var i = 0; i < this.stimuli.length; ++i){
    this.responses[i] = $("input[name='"+this.stimulusQuestionnaires[i].prefix +"_response']:checked").val();
  }
...
LikertMultiStimulusPage.prototype.store = function () {
  ...
  for(var i = 0; i < this.stimuli.length; ++i){
    ...
    if(this.responses[i] == undefined){
      ...
      rating.stimulusRating = this.responses[i];
    }
